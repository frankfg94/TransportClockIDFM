...
describe("network ghost canvas renderer", () => {
  ...
  it("limits visible and overscan tiles while keeping the count independent from DPR", () => {
    ...
    expect(
      retina.tiles.every(
        (tile) => tile.pixelWidth === Math.round(tile.width * retina.zoom * 2),
      ),
    ).toBe(true);
  ...
  it("keeps a GTFS bus stop and its Canvas trace in the same position at every zoom", () => {
    const line = createLine("bus:21", [createSegment("gtfs", 0.2, 0.3, 0.8, 0.7)]);
    line.stations = [
      { id: "gtfs:from", label: "From", x: 0.2, y: 0.3 },
      { id: "gtfs:to", label: "To", x: 0.8, y: 0.7 },
    ];

    for (const zoom of [1, 4, 20]) {
      const options = { ...createSceneOptions(), zoom };
      const scene = buildNetworkGhostCanvasScene([line], options);
      const plan = createNetworkGhostTilePlan(scene, {
        zoom,
        viewport: { x: 800, y: 360, width: 900, height: 620 },
        pixelRatio: 2,
        tileSize: 512,
        maxTiles: 16,
      });
      const traceStart = scene.lines[0].segments[0].points[0];
      const station = line.stations[0];
      const expectedSvgPoint = {
        x: options.paddingX + station.x * (options.viewBoxWidth - options.paddingX * 2),
        y: options.paddingY + station.y * (options.viewBoxHeight - options.paddingY * 2),
      };

      expect(traceStart).toEqual(expectedSvgPoint);
      expect({
        x: traceStart.x * zoom,
        y: traceStart.y * zoom,
      }).toEqual({
        x: expectedSvgPoint.x * zoom,
        y: expectedSvgPoint.y * zoom,
      });

      for (const tile of plan.tiles) {
        expect(getNetworkGhostTileCssRect(tile)).toEqual({
          x: tile.x,
          y: tile.y,
          width: tile.width,
          height: tile.height,
        });
        expect(tile.pixelWidth).toBe(Math.round(tile.width * plan.zoom * plan.pixelRatio));
        expect(tile.pixelHeight).toBe(Math.round(tile.height * plan.zoom * plan.pixelRatio));
      }
    }
  });
