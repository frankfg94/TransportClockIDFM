...
describe("network ghost geometry", () => {
  ...
  it("replaces stale direct topology with provider-owned trace segments", () => {
    ...
    expect(resolved.segments[0].polyline).toHaveLength(3);
    resolved.stations.forEach((station) => {
      const trace = resolved.segments[0].polyline!;
      const crossProduct =
        (station.x - trace[0].x) * (trace[1].y - trace[0].y) -
        (station.y - trace[0].y) * (trace[1].x - trace[0].x);
      expect(Math.abs(crossProduct)).toBeLessThan(0.000001);
    });
    expect(resolved.anchorX).toBe(resolved.stations[0].x);
    expect(resolved.anchorY).toBe(resolved.stations[0].y);
  });
});
