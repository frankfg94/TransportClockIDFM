...
describe("pattern traffic calendar", () => {
    ...
    const events = createPatternTrafficCalendarEvents(
      [
        createDisruption("current", [
          { begin: "20260707T090000", end: "20260710T220000" },
        ]),
        createDisruption("expired", [
          { begin: "20260706T090000", end: "20260706T220000" },
        ]),
      ],
      stations,
      edges,
  ...
  it("keeps consecutive empty months through the final event month", () => {
    ...
    const events = createPatternTrafficCalendarEvents(
      [
        createDisruption("september", [
          { begin: "20260912T090000", end: "20260912T120000" },
        ]),
      ],
      stations,
      edges,
  ...
  it("filters disturbances without changing interruption events", () => {
    ...
    const disruptions = [
      createDisruption(
        "disturbance",
        [{ begin: "20260712T090000", end: "20260712T120000" }],
        { title: "Trafic perturb? entre Station A et Station D" },
      ),
      createDisruption("interruption", [
        { begin: "20260713T090000", end: "20260713T120000" },
      ]),
    ];
  ...
  it("deduplicates affected stations and lets interruptions win", () => {
    ...
    const events = createPatternTrafficCalendarEvents(
      [
        ...
        createDisruption("interruption", [
          { begin: "20260712T130000", end: "20260712T160000" },
        ]),
      ],
      stations,
      edges,
    ...
    expect(day?.interruptedStationLabels.length).toBe(
      new Set(day?.interruptedStationLabels).size,
    );
    expect(
      day?.disturbedStationLabels.some((station) =>
        day.interruptedStationLabels.includes(station),
      ),
    ).toBe(false);
  });
  it("downgrades a late-evening interruption while keeping unspecified hours full-day", () => {
    ...
    const events = createPatternTrafficCalendarEvents(
      [
        createDisruption(
          "evening",
          [{ begin: "20260712T224500", end: "20260713T030000" }],
          {
            title:
              "Trafic interrompu entre Station A et Station D a partir de 22h45, reprise a 3h",
          },
        ),
        createDisruption("full-day", [
          { begin: "20260714T000000", end: "20260714T235959" },
        ]),
      ],
      stations,
      edges,
    ...
    const evening = month.days.find(
      (candidate) => candidate.dateKey === "2026-07-12",
    );
    const fullDay = month.days.find(
      (candidate) => candidate.dateKey === "2026-07-14",
    );
