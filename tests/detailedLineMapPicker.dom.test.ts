...
describe("DetailedLineMapPicker sidebar", () => {
  it("opens the station addressed by the URL query", async () => {
  ...
  it("removes an unknown station query from the URL", async () => {
    routeState.query = { view: "map", station: "station:missing" };
  ...
  it("keeps the station query in sync with explorer station clicks", async () => {
    routeState.query = { view: "map" };
  ...
  it("loads ghost correspondences progressively and keeps active quays visible", async () => {
    ...
    const sharedGhostPath = wrapper.get(".network-ghost-line__segment").element;
    ...
    expect(wrapper.findAll(".network-ghost-line__segment")).toHaveLength(1);
    expect(
      wrapper.get(".network-ghost-line--active .network-ghost-line__segment").element,
    ).toBe(sharedGhostPath);
