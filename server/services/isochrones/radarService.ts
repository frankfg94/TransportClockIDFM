...
import type { GlobalIsochroneRequest, GlobalIsochroneMinutes } from "../../../src/features/transport-map/isochrones/contracts";
...
export async function selectServerStationIsochrones(
  origin: { lon: number; lat: number },
  env: NetexRuntimeEnv,
  minutes: readonly GlobalIsochroneMinutes[] = NEARBY_WALKING_MINUTES,
) {
  ...
  try {
    ...
    return await (await cached.value).selectOrigin(source, origin, minutes);
