...
export function createNetworkGhostTilePlan(
  ...
  let tileSize = Math.max(128, Math.floor(options.tileSize ?? DEFAULT_TILE_SIZE)) / zoom;
  const sceneWidth = scene.viewBoxWidth;
  const sceneHeight = scene.viewBoxHeight;
  const sceneViewport = {
    x: options.viewport.x / zoom,
    y: options.viewport.y / zoom,
    width: options.viewport.width / zoom,
    height: options.viewport.height / zoom,
  };

  while (
    countVisibleTiles(sceneViewport, tileSize, sceneWidth, sceneHeight) > maximumTiles &&
    tileSize * zoom < 2_048
  ) {
    tileSize *= 2;
  }
  ...
  const basePixelCount = coordinates.reduce((total, { column, row }) => {
    ...
    return (
      total +
      Math.min(tileSize, Math.max(0, sceneWidth - x)) *
        Math.min(tileSize, Math.max(0, sceneHeight - y)) *
        zoom *
        zoom
    );
  }, 0);
...
export function drawNetworkGhostCanvasTile(
  ...
  prepareNetworkGhostCanvasContext(context, tile, plan, true);

  tile.lines.forEach(({ line, segments }) => {
    drawNetworkGhostCanvasLine(context, line, segments, visualZoom, false);
  });
  context.globalAlpha = 1;
}

export function drawNetworkGhostCanvasLineHighlight(
  context: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D,
  tile: Pick<
    NetworkGhostCanvasTile,
    "x" | "y" | "width" | "height" | "pixelWidth" | "pixelHeight"
  >,
  plan: Pick<NetworkGhostCanvasTilePlan, "pixelRatio" | "zoom">,
  sceneLine: NetworkGhostCanvasLine,
  visualZoom = plan.zoom,
): void {
  const tileRect = getNetworkGhostTileCssRect(tile);
  if (!rectsIntersect(sceneLine.bounds, tileRect)) return;
  const segments = sceneLine.segments.filter((segment) =>
    rectsIntersect(segment.bounds, tileRect),
  );
  if (segments.length === 0) return;

  prepareNetworkGhostCanvasContext(context, tile, plan, false);
  drawNetworkGhostCanvasLine(context, sceneLine.line, segments, visualZoom, true);
  context.globalAlpha = 1;
}

export function getNetworkGhostTileCssRect(
...
function prepareNetworkGhostCanvasContext(
  context: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D,
  tile: Pick<NetworkGhostCanvasTile, "x" | "y" | "pixelWidth" | "pixelHeight">,
  plan: Pick<NetworkGhostCanvasTilePlan, "pixelRatio" | "zoom">,
  clear: boolean,
): void {
  const renderScale = plan.pixelRatio * plan.zoom;
  context.setTransform(1, 0, 0, 1, 0, 0);
  if (clear) context.clearRect(0, 0, tile.pixelWidth, tile.pixelHeight);
...
function drawNetworkGhostCanvasLine(
  context: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D,
  line: NetworkGhostLineView,
  segments: NetworkGhostCanvasSegment[],
  visualZoom: number,
  ...
  context.globalAlpha = highlighted ? 0.88 : line.isBus ? 0.3 : 0.58;
  context.lineWidth = getNetworkGhostStrokeWidth(line) / Math.max(visualZoom, 0.01);
  context.stroke();
}
...
function createCanvasTile(
  ...
  const width = Math.min(options.tileSize, Math.max(0, options.sceneWidth - x));
  const height = Math.min(options.tileSize, Math.max(0, options.sceneHeight - y));
  const sceneRect = { x, y, width, height };
  ...
  return {
    ...
    width,
    height,
    pixelWidth: Math.max(1, Math.round(width * options.zoom * options.pixelRatio)),
    pixelHeight: Math.max(1, Math.round(height * options.zoom * options.pixelRatio)),
    memoryBytes:
      Math.max(1, Math.round(width * options.zoom * options.pixelRatio)) *
      Math.max(1, Math.round(height * options.zoom * options.pixelRatio)) *
      4,
