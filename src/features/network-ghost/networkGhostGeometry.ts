...
export function applyNetworkGhostGeometry(
  ...
  const providerPolylines = providerSegments.flatMap((segment) =>
    segment.polyline ? [segment.polyline] : [],
  );
  const stations = line.stations.map((station) => {
    if (plan.topology === "provider") {
      const point = projectPointOntoLineGeometry(station, providerPolylines);
