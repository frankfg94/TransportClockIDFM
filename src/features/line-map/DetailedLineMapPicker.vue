...
interface ToggleStopDetailsOptions {
  syncStationQuery?: boolean;
}
...
const GHOST_BACKGROUND_TARGET = "#line-map-network-ghost-background-layer";
...
function selectStop(stop: LineMapStopView): void {
  if (isExplorerMode.value) {
    toggleStopDetails(stop, { syncStationQuery: true });
  }
...
function toggleStopDetails(
  stop: LineMapStopView,
  options: ToggleStopDetailsOptions = {},
): void {
  favoriteError.value = "";
...
function openStationFromQuery(map: LineMapViewModel): void {
  if (!isExplorerMode.value) {
    return;
  }
  ...
  const stop = map.stops.find((item) => item.id === stationId);
  if (!stop) {
    updateStationQuery();
    return;
  }
...
function closeSidebarFromRoute(): void {
  closeSidebar();
  updateStationQuery();
}
        ...
        >
          ...
          >
            <TransitNetworkGhostLayer
              ...
              :background-target="GHOST_BACKGROUND_TARGET"
          ...
          <g
            id="line-map-network-ghost-background-layer"
            class="line-map-network-ghost-background-layer"
            aria-hidden="true"
          ></g>
