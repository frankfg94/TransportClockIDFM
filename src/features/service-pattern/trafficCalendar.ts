...
export function createPatternTrafficCalendarEvents(
  ...
  return disruptions
    .flatMap((disruption) => {
      const impactAnalysis = analyzeTrafficImpacts(
        [disruption],
        stations,
        edges,
      );
      ...
      const selectedSegments = impactAnalysis.segments.filter(
        (segment) =>
          impactScope === "all-impacts" || segment.kind === "interruption",
      );
      ...
      const fallbackStationKeys =
        stationKinds.size === 0
          ? getEndpointKeys(Array.from(edgeKeys), edges)
          : [];
      ...
      return periods.flatMap((period, periodIndex) => {
        ...
        if (end) {
          event.restartTimeLabel =
            restartClock?.label ?? formatClockTime(end);
        }
  ...
  return {
    ...
    days: Array.from({ length: 42 }, (_, index) => {
      ...
      const dayEvents = events.filter((event) =>
        trafficCalendarEventOverlaps(event, dateStart, dateEnd),
      );
      ...
      const fallbackStationKeys =
        affectedStationKeys.length === 0
          ? Array.from(
              new Set(dayEvents.flatMap((event) => event.fallbackStationKeys)),
            )
          : [];
      const temporalMultipliersByStationKey =
        createTrafficTemporalMultipliersByStation(
      ...
      const durationMinutes = calculateTrafficCalendarDurationMinutes(
        dayEvents
          .filter(
            (
              event,
            ): event is PatternTrafficCalendarEvent & { end: Date } =>
              Boolean(event.end),
          )
          .map((event) => ({
            start: new Date(
              Math.max(event.start.getTime(), dateStart.getTime()),
            ),
            end: new Date(
              Math.min(event.end.getTime(), dateEnd.getTime()),
            ),
          })),
      );
      const singleEvent =
      ...
      const day: PatternTrafficCalendarDay = {
        ...
        events: dayEvents,
        impactCount: new Set(
          dayEvents.map((event) => event.disruption.id),
        ).size,
        ...
        affectedSegmentLabels: Array.from(
          new Set(
            dayEvents.flatMap((event) => event.affectedSegmentLabels),
          ),
        ).sort(localeSort),
      };
...
export function getPatternTrafficCalendarBounds(
  ...
  const lastEventTime = events.reduce(
    (latest, event) =>
      Math.max(latest, event.end?.getTime() ?? event.start.getTime()),
    firstMonth.getTime(),
  );
...
function trafficCalendarEventOverlaps(
  ...
  return (
    event.start.getTime() < nextDayStart.getTime() &&
    eventEnd >= dayStart.getTime()
  );
}
  ...
  return new Map(
    stationKeys.map((stationKey) => {
      const windows = events
        ...
        .map((event) =>
          getEventTrafficImpactTimeWindow(event, dayStart, nextDayStart),
        );
      return [
        stationKey,
        calculateTrafficImpactTemporalMultiplier(windows).multiplier,
      ];
    }),
  );
}
...
function getEventTrafficImpactTimeWindow(
  ...
  if (
    ...
    endTime >= nextDayStart.getTime() - 1
  ) {
    return undefined;
  }
  ...
  return {
    startMinute:
      startTime > dayStart.getTime()
        ? getLocalMinuteOfDay(event.start)
        : 0,
    endMinute:
